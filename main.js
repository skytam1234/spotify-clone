  ...
  playlistDropdown.addEventListener("click", async function (e) {
    ...
    if (createPlaylist) {
      ...
      try {
        ...
        toggleMainContent(false, false, false, false, true, true, true, false);
      } catch (error) {}
    }
  });
    ...
    if (libraryItem) {
      ...
      try {
        ...
        if (tracks.tracks.length > 0) {
          ...
          artistHeroSubtitle.textContent = playlist.description || playlist.bio;
          toggleMainContent(
            false,
            false,
            false,
  ...
  contentWrapper.addEventListener("click", async (e) => {
    ...
    if (item) {
      ...
      if (item.dataset.type === "track") {
        ...
        if (type === "artist") {
          ...
          const res = await httpRequest.get(`${type + "s/" + id}/tracks`);
          tracks = res.tracks;
        }
    ...
    if (followBtnLarge) {
      ...
      if (itemData.type === "playlist") {
        try {
          const res = await httpRequest.post(`playlists/${itemData.id}/follow`);
      ...
      if (itemData.type === "album") {
        try {
          const res = await httpRequest.post(`albums/${itemData.id}/like`);
          console.log(res);
          iziToast.info({
            title: "Thông Báo",
            message: "Thành công",
    ...
    if (unfollowBtnLarge) {
      ...
      if (itemData.type === "artist") {
        try {
          const res = await httpRequest.delete(`artists/${itemData.id}/follow`);
          console.log(res);
      ...
      if (itemData.type === "album") {
        try {
          const res = await httpRequest.delete(`albums/${itemData.id}/like`);
          console.log(res);
          iziToast.info({
            title: "Thông Báo",
            message: "Thành công!",
    ...
    timeDelay = setTimeout(async () => {
      ...
      if (value.length > 0) {
        ...
        listPlaylist.length > 0
          ? (contentPlaylistSection.textContent = "Playlist dành cho bạn")
          : (contentPlaylistSection.textContent = "Playlist dành cho bạn");
...
async function playlistTrending(playlists) {
  const contentPlaylistGrid = document.querySelector(`.content-playlist-grid`);
  let html = "";
  playlists.forEach((playlist) => {
    const item = `<div
      ...
      playlist.title
                                ...
                                <div class="playlist-card-info">
                                    ...
                                    <p class="playlist-card-artist">
                                        ${escapeHtml(
                                          playlist.additional_info.creator_name
                                        )}
                                    </p>
                                </div>
                            </div>`;
...
const musicPlayer = {
  ...
  setupEventListeners() {
    ...
    this.audioPlayer.ontimeupdate = () => {
      ...
      const value =
        (this.audioPlayer.currentTime - (this.audioPlayer.currentTime % 60)) /
          60 +
        ":" +
